...
const ProductsSection: React.FC = () => {
  ...
  // Derive the actual category to display
  const selectedCategory =
    activeCategory || (dynamicCategories.length > 0 ? dynamicCategories[0].id : '');
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {isLoading ? (
            <div className="flex md:grid md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-10 overflow-hidden">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="min-w-[85vw] md:min-w-0 aspect-[4/5] bg-gray-100 animate-pulse rounded-lg"
                />
              ))}
            </div>
          ) : (
        ...
        {filteredProducts.length > 4 && (
          <div className="mt-16 flex justify-center">
            <motion.a
              href={`/productos?category=${selectedCategory}`}
