...
function CatalogContent() {
  ...
  const [activeCategoryState, setActiveCategoryState] = useState(initialCategory);
  ...
  const activeCategory =
    ...
    activeCategoryState ||
    (dynamicCategories.length > 0 ? dynamicCategories[0].id : '');

  const setActiveCategory = (cat: string) => {
    setActiveCategoryState(cat);
    router.push(`/productos?category=${cat}`, { scroll: false });
  };
